...
% Differences of triangle coordinates in x and y direction
...
% Assembly of load vector (Neumann / Robin BC)
r_row = repmat((1/2).*E_len,1,2);
r     = g.*sparse(oo,pp,r_row(:),mesh.Nn,1);

% Assembly of load vector (Dirichlet)
% (Dirichlet lift ansatz)
d_ = zeros(mesh.Nn,1);
d_(dir) = g(dir);
d = -A*d_;

% Final stifness matrix and load vector
S = A+2*B;
f = f+2*r+d;
